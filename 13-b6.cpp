...
void TString::set(const char *a)
{
	...
	if (a == NULL || strlen(a) == 0) {
		...
		int i = 0;
		len = strlen(a);
		content = new(nothrow)char[strlen(a) + 1];
		if (content == NULL)
			exit(-1);
		while (a[i] != '\0') {
			content[i] = a[i];
...
/***************************************************************************
  函数名称：set
  功    能：将一个对象的字符串赋值给另一个对象
  输入参数：TString
  返 回 值：void
  说    明：set()函数的体外实现
***************************************************************************/
void TString::set(TString b)
{
	if (content)
		delete[]content;
	if (b.content == NULL || strlen(b.content) == 0) {
		len = 0;
		content = NULL;
	}
	else {
		...
		content = new(nothrow)char[b.len + 1];
		if(content==NULL)
			exit(-1);
		len = b.len;
		while (b.content[i] != '\0') {
